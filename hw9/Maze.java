...
public class Maze {
	...
	public Maze(int horizontalSize, int verticalSize) {
		...
		int w = horiz * (vert - 1) + (horiz - 1) * vert;
		int[][] walls = new int[w][3];
		if (vert > 1) {
			for (j = 0; j < vert - 1; j++) {
				for (i = 0; i < horiz; i++) {
					walls[j * horiz + i][0] = i;
					walls[j * horiz + i][1] = j;
					// 0 indicate this is hWall
					walls[j * horiz + i][2] = 0;
				}
			}
		}
		if (horiz > 1) {
			for (i = 0; i < horiz - 1; i++) {
				for (j = 0; j < vert; j++) {
					walls[vert * i + j + (horiz * (vert - 1))][0] = i;
					walls[vert * i + j + (horiz * (vert - 1))][1] = j;
					// 1 indicate this is vWall
					walls[vert * i + j + (horiz * (vert - 1))][2] = 1;
				}
			}
		}
		int k = w;
		//randomize the array walls.
		while (k != 1) {
			swap(walls[k - 1], walls[randInt(k)]);
			k--;
		}
		//start to make the maze now.
		for (k = 0; k < w; k++) {
			int wi = walls[k][0];// get the i position from array
			int wj = walls[k][1];// get the j position from array
			// this is hWall
			if (walls[k][2] == 0) {
				// check if cells(wi,wj) and cells(wi,wj+1) is in the same set,
				// if not union them.
				if (cells.find(wi * vert + wj) != cells.find(wi * vert + wj + 1)) {
					hWalls[wi][wj] = false;
					cells.union(cells.find(wi * vert + wj), cells.find(wi * vert + wj + 1));
				}
				// this is vWall
			} else {
				// check if cells(wi,wj) and cells(wi+1,wj) is in the same set,
				// if not union them.
				if (cells.find(wi * vert + wj) != cells.find((wi + 1) * vert + wj)) {
					vWalls[wi][wj] = false;
					cells.union(cells.find(wi * vert + wj), cells.find((wi + 1) * vert + wj));
				}
			}
		}
	}

	// swap two array,x and y should have same length.
