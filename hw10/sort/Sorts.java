...
public class Sorts {
	...
	private final static int NUMBERLENGTH = 8;
	private final static int DIGIT=16;
	...
	public static int[] countingSort(int[] keys, int whichDigit) {
		...
		// a new array shiftKey which is the array keys right shift whichDigit.
		int[] shiftKeys = new int[keys.length];
		for (int j = 0; j < keys.length; j++) {
			shiftKeys[j] = keys[j];
			for (int i = 0; i < whichDigit; i++) {
				shiftKeys[j] = shiftKeys[j] >> 4;
			}
		}
		//begin counting
		...
		for (int i = 0; i < shiftKeys.length; i++) {
			count[shiftKeys[i] & MASK]++;
		}
		//count[i] contains the number of keys less than i by the sort digit
		int total = 0;
		for (int i = 0; i < count.length; i++) {
			int c = count[i];
			count[i] = total;
		...
		//put the keys in the right position
		int[] result = new int[shiftKeys.length];
		for (int i = 0; i < shiftKeys.length; i++) {
			result[count[shiftKeys[i] & MASK]] = keys[i];
			count[shiftKeys[i] & MASK]++;
		}
		return result;
	}
	...
	public static int[] radixSort(int[] keys) {
		...
		// System.out.print("Sort by 0 digit: ");
		// yell(result);
		for (int i = 1; i < NUMBERLENGTH; i++) {
			result = countingSort(result, i);
			// System.out.print("Sort by "+i+" digit: ");
			// yell(result);
		}
		return result;
	}
